...
const imageInputMode = ref<'link' | 'local' | 'library'>('link')
const fileInputRef = ref<HTMLInputElement | null>(null)
const isUploading = ref(false)

function handleSelectFromLibrary(urls: string | string[]) {
  if (Array.isArray(urls)) {
    productImages.value = [...productImages.value, ...urls]
  } else if (typeof urls === 'string') {
    productImages.value = [...productImages.value, urls]
  }
  toast.success('เพิ่มรูปภาพแล้ว!', 'นำเข้ารูปภาพจากคลังเรียบร้อย')
}
          ...
          <div class="flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                class="px-3.5 py-1.5 rounded-xl font-black text-xs bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800/60 transition flex items-center gap-1.5 cursor-pointer shadow-sm"
              >
                <Icon name="lucide:folder-open" class="w-3.5 h-3.5" />
                <span>เปิดคลังรูปภาพ</span>
              </button>

              <div class="flex bg-slate-200/70 dark:bg-[#0a0a0a] p-1 rounded-xl border border-slate-200 dark:border-[#212327] text-xs">
                <button type="button" @click="imageInputMode = 'link'" :class="imageInputMode === 'link' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'" class="px-3 py-1 rounded-lg font-bold transition flex items-center gap-1.5 cursor-pointer">
                  <Icon name="lucide:link" class="w-3.5 h-3.5" /> ลิงก์
                </button>
                <button type="button" @click="imageInputMode = 'local'" :class="imageInputMode === 'local' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'" class="px-3 py-1 rounded-lg font-bold transition flex items-center gap-1.5 cursor-pointer">
                  <Icon name="lucide:upload" class="w-3.5 h-3.5" /> อัปโหลด
                </button>
              </div>
            </div>
          </div>
