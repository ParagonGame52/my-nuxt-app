        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
          <tr v-for="user in users" :key="user.id" class="hover:bg-slate-50/50 dark:hover:bg-[#1a1c20]/20 transition">
            ...
            <td class="p-4">
              <span
                :class="{
                  'bg-cyan-100 dark:bg-cyan-950/50 text-cyan-700 dark:text-cyan-300':   ((user as any).tier || 'bronze').toLowerCase() === 'platinum',
                  'bg-yellow-100 dark:bg-yellow-950/50 text-yellow-600 dark:text-yellow-400': ((user as any).tier || 'bronze').toLowerCase() === 'gold',
                  'bg-slate-200 dark:bg-slate-700/60 text-slate-600 dark:text-slate-300': ((user as any).tier || 'bronze').toLowerCase() === 'silver',
                  'bg-orange-100 dark:bg-orange-950/40 text-orange-700 dark:text-orange-400': ((user as any).tier || 'bronze').toLowerCase() === 'bronze',
                }"
            ...
            <td class="p-4 text-right">
              <div class="flex justify-end gap-2 items-center">
                ...
                <!-- Delete User (Strictly Super Admin only) -->
                <button
                ...
                <div
                  v-else
                  class="p-2 border border-slate-200 dark:border-[#212327] rounded-xl text-slate-400 dark:text-slate-600 cursor-not-allowed opacity-50 flex items-center justify-center"
                  title="เฉพาะแอดมินสูงสุด (Super Admin) เท่านั้นที่สามารถลบบัญชีได้"
                >
