...
<template>
  <div class="bg-white dark:bg-[#191919] border border-slate-200 dark:border-[#212327] rounded-3xl overflow-hidden shadow-sm">
    <div v-if="withdrawalRequests.length === 0" class="p-12 text-center text-slate-500">
      ...
      <p class="font-bold text-sm">ไม่มีคำขอถอนเงิน</p>
    </div>
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left border-collapse text-xs">
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
          <tr v-for="req in withdrawalRequests" :key="req.id" class="hover:bg-slate-50/50 dark:hover:bg-[#1a1c20]/20 transition">
            <td class="p-4">
              <p class="font-bold text-slate-800 dark:text-slate-200">{{ req.user_name }}</p>
              <p class="text-[10px] text-slate-400">{{ req.user_email }}</p>
            </td>
            <td class="p-4 font-black text-rose-600 dark:text-rose-400">฿{{ Number(req.amount).toLocaleString() }}</td>
            <td class="p-4">
              <p class="font-bold text-slate-800 dark:text-slate-200">{{ req.bank_name }}</p>
              <p class="text-[10px] text-slate-400 font-mono">{{ req.bank_account }}</p>
            </td>
            ...
            <td class="p-4">
              ...
              <p v-if="req.note" class="text-[10px] text-slate-400 mt-1">หมายเหตุ: {{ req.note }}</p>
            </td>
            <td class="p-4 text-right">
              <div class="flex items-center justify-end gap-2">
                <template v-if="req.status === 'pending'">
                  ...
                  <button @click="emit('reject', req.id)" class="bg-amber-600 hover:bg-amber-700 text-white font-bold px-3 py-1.5 rounded-xl text-xs transition flex items-center gap-1 cursor-pointer">
                    <Icon name="lucide:x" class="w-3.5 h-3.5" />
                    <span>ปฏิเสธ</span>
                  </button>
                </template>
                <span v-else class="text-slate-400 text-xs mr-1">ดำเนินการแล้ว</span>
                <button
                  @click="emit('delete', req.id)"
                  title="ลบคำขอถอนเงินนี้"
                  class="w-7 h-7 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 flex items-center justify-center transition cursor-pointer"
                >
