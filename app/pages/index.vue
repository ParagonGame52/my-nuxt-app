...
const activities = computed(() => {
  const list = activitiesData.value?.activities || []
  if (list.length > 0) {
    return list.slice(0, 3)
  }
  return [
    { id: 1, text: 'สุ่มได้ เสื้อกันหนาว Oversize Streetwear', userName: 'ลูกค้า', time: 'เมื่อสักครู่', img: 'https://images.unsplash.com/photo-1556821840-3a63f95609a7?w=100&q=80' },
    { id: 2, text: 'ซื้อ Minimal Blazer Set', userName: 'ลูกค้า', time: '12 นาทีที่แล้ว', img: 'https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=100&q=80' },
    { id: 3, text: 'สุ่มได้ กางเกง Cargo ขากระบอกใหญ่', userName: 'ลูกค้า', time: '25 นาทีที่แล้ว', img: 'https://images.unsplash.com/photo-1541099649105-f69ad21f3246?w=100&q=80' }
  ]
})
      ...
      <div class="bg-white dark:bg-[#191919] border border-slate-300 dark:border-[#212327] rounded-3xl p-6 shadow-sm dark:shadow-2xl/20 space-y-5 transition-all duration-300">
        ...
        <div class="space-y-3.5">
          <div v-for="act in activities" :key="act.id" class="flex items-center space-x-4 p-3 rounded-2xl bg-slate-50/70 dark:bg-[#0a0a0a] hover:bg-slate-100 dark:hover:bg-[#1a1c20] transition duration-200 border border-slate-200/80 dark:border-[#212327]">
            <div class="w-11 h-11 bg-slate-200 dark:bg-[#1a1c20] rounded-xl overflow-hidden flex-shrink-0 border border-slate-300 dark:border-[#212327]">
              <img :src="act.img" class="w-full h-full object-cover animate-fade-in" :alt="act.text">
            </div>
