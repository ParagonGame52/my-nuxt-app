...
// ─── Profile Banner & Theme Customizer ─────────────
...
const THEME_PRESETS = [
  { id: 'cyberpunk-diamond', name: '💎 Cyberpunk Diamond', color: 'from-[#070b14] via-[#0f172a] to-[#1e1b4b]', border: 'border-2 border-cyan-500/50', text: 'text-cyan-300', glow: 'shadow-cyan-500/20', preview: 'bg-gradient-to-r from-[#070b14] via-[#0f172a] to-[#1e1b4b]' },
  { id: 'midnight-nebula', name: '🌌 Midnight Nebula', color: 'from-[#110726] via-[#240b36] to-[#0d0d2b]', border: 'border-2 border-pink-500/40', text: 'text-pink-300', glow: 'shadow-pink-500/20', preview: 'bg-gradient-to-r from-[#110726] via-[#240b36] to-[#0d0d2b]' },
  { id: 'sunset-flare', name: '🌋 Sunset Flare', color: 'from-[#2d0b00] via-[#591a05] to-[#1a0802]', border: 'border-2 border-amber-500/40', text: 'text-amber-300', glow: 'shadow-amber-500/20', preview: 'bg-gradient-to-r from-[#2d0b00] via-[#591a05] to-[#1a0802]' },
  { id: 'matrix-emerald', name: '🌲 Matrix Emerald', color: 'from-[#021b14] via-[#043324] to-[#01140e]', border: 'border-2 border-emerald-500/40', text: 'text-emerald-300', glow: 'shadow-emerald-500/20', preview: 'bg-gradient-to-r from-[#021b14] via-[#043324] to-[#01140e]' },
  { id: 'stealth-carbon', name: '🖤 Stealth Carbon', color: 'from-[#0a0a0a] via-[#141414] to-[#1e1e1e]', border: 'border-2 border-zinc-500/40', text: 'text-zinc-300', glow: 'shadow-zinc-500/20', preview: 'bg-gradient-to-r from-[#0a0a0a] via-[#141414] to-[#1e1e1e]' },
  { id: 'royal-gold', name: '👑 Royal Gold', color: 'from-[#241a02] via-[#4a3605] to-[#1a1301]', border: 'border-2 border-yellow-500/40', text: 'text-yellow-300', glow: 'shadow-yellow-500/20', preview: 'bg-gradient-to-r from-[#241a02] via-[#4a3605] to-[#1a1301]' },
  { id: 'ocean-deep', name: '🌊 Ocean Deep', color: 'from-[#031b2e] via-[#083b66] to-[#041c30]', border: 'border-2 border-blue-500/40', text: 'text-blue-300', glow: 'shadow-blue-500/20', preview: 'bg-gradient-to-r from-[#031b2e] via-[#083b66] to-[#041c30]' },
  { id: 'classic-blue', name: '🔵 Dip Drip Classic', color: 'from-blue-600 via-indigo-600 to-purple-700', border: 'border-2 border-white/20', text: 'text-white', glow: 'shadow-blue-600/20', preview: 'bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700' }
]

function openThemeModal() {
  selectedTheme.value = (authStore.user as any)?.banner_theme || (userTier.value === 'diamond' ? 'cyberpunk-diamond' : 'classic-blue')
  customBannerUrl.value = (authStore.user as any)?.banner_url || ''
  ...
  try {
    await $fetch('/api/auth/profile', {
      ...
      body: {
        ...
        banner_theme: selectedTheme.value,
        banner_url: customBannerUrl.value
      }
    })
    await authStore.fetchMe()
    showThemeModal.value = false
    toast.success('บันทึกพื้นหลังสำเร็จ!', 'เปลี่ยนธีมพื้นหลังการ์ดโปรไฟล์เรียบร้อยแล้ว')
  } catch (e: any) {
    toast.error('บันทึกไม่สำเร็จ', e.data?.statusMessage || e.message)
  } finally {
...
async function onBannerFileChange(e: Event) {
  ...
  if (file.size > 8 * 1024 * 1024) {
    toast.error('ไฟล์มีขนาดใหญ่เกินไป', 'ขนาดไฟล์สูงสุดไม่เกิน 8MB')
    return
  }
  ...
  reader.onload = (ev) => {
    customBannerUrl.value = ev.target?.result as string
  }
  ...
  try {
    ...
    const res = await $fetch<any>('/api/auth/banner', {
      method: 'POST',
      body: formData
    })
    const newUrl = res.url || res.bannerUrl || ''
    if (newUrl) {
      customBannerUrl.value = newUrl
      await authStore.fetchMe()
      toast.success('อัปโหลดรูปภาพสำเร็จ!', 'เปลี่ยนรูปภาพพื้นหลังการ์ดเรียบร้อยแล้ว')
    }
  } catch (e: any) {
    toast.error('อัปโหลดรูปภาพไม่สำเร็จ', e.data?.statusMessage || e.message)
  } finally {
  ...
  switch (userTier.value) {
    case 'diamond':
      return { 
        ...
        badgeBg: 'bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 text-white shadow-lg shadow-cyan-500/25 border-cyan-300',
        cardBg: 'bg-gradient-to-br from-[#070b14] via-[#0f172a] to-[#1e1b4b] border-2 border-cyan-500/40 shadow-2xl shadow-cyan-500/20',
        nextTier: null, 
        nextTarget: 0, 
        discount: '25%', 
        multiplier: '10x',
    ...
    case 'platinum':
      return { 
        ...
        badgeBg: 'bg-white/20 backdrop-blur-sm border-white/30 text-amber-300',
        cardBg: 'bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 border border-slate-700/60 shadow-xl shadow-slate-900/40',
        nextTier: null, 
        nextTarget: 0, 
        discount: '15%', 
        multiplier: '5x',
    ...
    case 'gold':
      return { 
        ...
        badgeBg: 'bg-white/20 backdrop-blur-sm border-white/30 text-yellow-300',
        cardBg: 'bg-gradient-to-br from-[#1a1405] via-[#291e0a] to-[#1f1606] border border-amber-500/40 shadow-xl shadow-amber-500/10',
        nextTier: 'Platinum', 
        nextTarget: 15000, 
        discount: '10%', 
        multiplier: '3x',
    ...
    case 'silver':
      return { 
        ...
        badgeBg: 'bg-white/20 backdrop-blur-sm border-white/30 text-slate-200',
        cardBg: 'bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] border border-slate-600/40 shadow-xl',
        nextTier: 'Gold', 
        nextTarget: 5000, 
        discount: '5%', 
        multiplier: '2x',
    ...
    default:
      return { 
        ...
        badgeBg: 'bg-white/20 backdrop-blur-sm border-white/30 text-amber-200',
        cardBg: 'bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 border border-white/10 shadow-xl shadow-blue-600/20',
        nextTier: 'Silver', 
        nextTarget: 1000, 
        discount: '0%', 
        multiplier: '1x',
...
const heroCardStyle = computed(() => {
  const customUrl = customBannerUrl.value || (authStore.user as any)?.banner_url || ''
  const themeId = selectedTheme.value || (authStore.user as any)?.banner_theme
  const foundTheme = THEME_PRESETS.find(t => t.id === themeId)

  let bgClass = foundTheme ? `bg-gradient-to-br ${foundTheme.color} ${foundTheme.border} ${foundTheme.glow}` : tierBadgeInfo.value.cardBg
...
<template>
    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div v-else class="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <div class="xl:col-span-2 space-y-6">
          ...
          <!-- Profile Hero Card -->
          <div :class="heroCardStyle.bgClass" class="rounded-3xl p-6 text-white relative overflow-hidden transition-all duration-500 shadow-2xl">
            <!-- Custom Background Image Overlay with high visibility & contrast -->
            <div
              v-if="heroCardStyle.customUrl"
              class="absolute inset-0 bg-cover bg-center z-0 pointer-events-none transition-all duration-500"
              :style="{ backgroundImage: `url(${heroCardStyle.customUrl})` }"
            >
              <div class="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/60 to-slate-950/40 backdrop-blur-[0.5px]" />
            </div>

            <!-- Ambient Lighting & Glow effects -->
            <div v-if="tierBadgeInfo.isAdminExclusive" class="absolute -top-14 -right-14 w-52 h-52 bg-cyan-500/20 rounded-full blur-3xl pointer-events-none animate-pulse" />
            <div v-if="tierBadgeInfo.isAdminExclusive" class="absolute -bottom-12 -left-12 w-48 h-48 bg-indigo-600/30 rounded-full blur-3xl pointer-events-none" />
            <template v-else>
              <div class="absolute -top-10 -right-10 w-40 h-40 bg-white/10 rounded-full blur-xl pointer-events-none" />
              <div class="absolute -bottom-6 -left-6 w-28 h-28 bg-white/10 rounded-full blur-xl pointer-events-none" />
            </template>

            <!-- Top Action Row: Profile Tag + Change Background Button -->
            <div class="relative z-10 flex items-center justify-between gap-2 mb-4">
              <span class="text-[10px] uppercase tracking-wider font-black px-2.5 py-0.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center gap-1">
                <Icon name="lucide:sparkles" class="w-3 h-3 text-cyan-300" />
                <span>VIP PROFILE CARD</span>
              </span>
              <button
                @click="openThemeModal"
                type="button"
                class="px-2.5 py-1 bg-black/40 hover:bg-black/60 active:scale-95 backdrop-blur-md rounded-xl border border-white/20 text-[11px] font-bold text-white flex items-center gap-1.5 transition-all shadow-md cursor-pointer hover:border-cyan-400/50"
                title="เปลี่ยนธีมหรือภาพพื้นหลังการ์ดโปรไฟล์"
              >
                <Icon name="lucide:palette" class="w-3.5 h-3.5 text-cyan-300" />
            ...
            <div class="relative z-10 flex items-center gap-5">
              ...
              <div class="relative flex-shrink-0">
                <div
                  :class="tierBadgeInfo.isAdminExclusive ? 'border-2 border-cyan-400 shadow-xl shadow-cyan-500/30 ring-4 ring-cyan-500/20 bg-gradient-to-br from-cyan-400 via-blue-600 to-indigo-950' : 'border-2 border-white/30 shadow-lg bg-white/20'"
                ...
                >
                  ...
                  <div v-else class="w-full h-full flex items-center justify-center text-3xl font-black">
                    {{ userInitials }}
                  </div>
                </div>
                <button
                  ...
                  class="absolute -bottom-1.5 -right-1.5 w-8 h-8 bg-white rounded-xl flex items-center justify-center shadow-lg hover:scale-105 transition-transform cursor-pointer"
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <span :class="tierBadgeInfo.badgeBg" class="text-xs md:text-sm font-black px-3 py-1 rounded-full border flex items-center gap-1.5 transition-all">
                    <Icon :name="tierBadgeInfo.icon" class="w-3.5 h-3.5" :class="tierBadgeInfo.isAdminExclusive ? 'text-cyan-200 animate-pulse' : 'text-amber-300'" />
            ...
            <p class="relative z-10 text-[10px] text-blue-200/70 mt-4 text-center flex items-center justify-center gap-1">
          ...
          <div class="bg-white dark:bg-[#191919] rounded-3xl p-6 border border-slate-200 dark:border-[#212327] shadow-sm space-y-4">
            ...
            <div v-else-if="tierBadgeInfo.isAdminExclusive" class="bg-gradient-to-r from-cyan-900/40 via-blue-900/30 to-indigo-900/40 p-3.5 rounded-2xl border border-cyan-500/40 text-center text-xs font-black text-cyan-300 flex items-center justify-center gap-1.5 shadow-sm">
              <Icon name="lucide:gem" class="w-4 h-4 text-cyan-400 animate-pulse" />
              <span>👑 ระดับสูงสุดพิเศษเฉพาะผู้ดูแลระบบ (Admin Diamond Tier)</span>
            </div>
  ...
  <!-- ─── Profile Card Background & Theme Customizer Modal ─── -->
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showThemeModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="showThemeModal = false">
        <div class="bg-white dark:bg-[#191919] rounded-3xl shadow-2xl w-full max-w-lg p-6 sm:p-7 border border-slate-200 dark:border-[#212327] space-y-5 animate-scale-up">
          
          <!-- Header -->
          <div class="flex items-center justify-between border-b border-slate-200 dark:border-[#212327] pb-3">
            <div class="flex items-center gap-2.5">
              <div class="w-9 h-9 rounded-xl bg-cyan-500/10 text-cyan-500 flex items-center justify-center">
                <Icon name="lucide:palette" class="w-5 h-5" />
              </div>
              <div>
                <h3 class="text-base font-black text-slate-800 dark:text-white">ปรับแต่งพื้นหลังการ์ดโปรไฟล์</h3>
                <p class="text-[10px] text-slate-400 font-bold">เลือกสไตล์ธีมสีหรือใส่รูปภาพวอลเปเปอร์ที่ต้องการ</p>
              </div>
            </div>
            <button @click="showThemeModal = false" class="w-8 h-8 rounded-xl bg-slate-100 dark:bg-[#0a0a0a] flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-700 transition cursor-pointer">
              <Icon name="lucide:x" class="w-4 h-4 text-slate-500" />
            </button>
          </div>
          ...
          <div class="space-y-2.5">
            <label class="block text-xs font-black text-slate-700 dark:text-slate-200">1. เลือกธีมสีพรีเซ็ต (Color Theme Presets)</label>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <button
                ...
                :class="[
                  selectedTheme === t.id ? 'ring-2 ring-cyan-500 scale-[1.03] shadow-md' : 'opacity-85 hover:opacity-100',
                  t.preview
                ]"
                class="p-2.5 rounded-2xl text-left border border-white/10 text-white transition-all cursor-pointer relative overflow-hidden h-14 flex flex-col justify-end"
              >
                <div v-if="selectedTheme === t.id" class="absolute top-1.5 right-1.5 w-4 h-4 rounded-full bg-cyan-400 text-slate-950 flex items-center justify-center text-[10px] font-black">
                  ✓
                </div>
                <p class="text-[10px] font-black truncate drop-shadow">{{ t.name }}</p>
              </button>
            </div>
          </div>

          <!-- Section 2: Custom Wallpaper Image -->
          <div class="space-y-2.5 pt-2 border-t border-slate-100 dark:border-[#212327]">
            <div class="flex items-center justify-between">
              <label class="block text-xs font-black text-slate-700 dark:text-slate-200">2. หรือเลือกรูปภาพพื้นหลังของตัวเอง (Custom Wallpaper Image)</label>
              <button
                v-if="customBannerUrl"
                type="button"
                @click="clearCustomBanner"
                class="text-[10px] font-bold text-rose-500 hover:text-rose-600 flex items-center gap-1 cursor-pointer"
              >
            ...
            <div class="flex gap-2">
              <input
                ...
                type="url"
                placeholder="วางลิงก์รูปภาพ หรือกดปุ่มเลือกรูปจากเครื่อง 👉"
                class="flex-1 bg-slate-50 dark:bg-[#0a0a0a] border border-slate-200 dark:border-[#212327] rounded-2xl px-4 py-2.5 text-xs font-bold text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 transition"
              />
              <button
                type="button"
                @click="triggerBannerPick"
                :disabled="uploadingBanner"
                class="px-3.5 py-2.5 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-600 dark:text-cyan-300 text-xs font-black rounded-2xl border border-cyan-500/30 transition flex items-center gap-1.5 cursor-pointer shadow-sm shrink-0"
                title="เลือกรูปภาพจากเครื่องของคุณ"
              >
                <div v-if="uploadingBanner" class="w-4 h-4 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" />
                <template v-else>
            ...
            <div v-if="customBannerUrl" class="relative rounded-2xl overflow-hidden h-20 border border-slate-200 dark:border-[#212327] mt-2 group">
              <img :src="customBannerUrl" alt="Banner Preview" class="w-full h-full object-cover" />
              <div class="absolute inset-0 bg-slate-950/60 flex items-center justify-between px-4 text-white">
                <span class="text-[11px] font-black flex items-center gap-1.5">
                  <Icon name="lucide:image" class="w-3.5 h-3.5 text-cyan-400" />
                  <span>ตัวอย่างภาพพื้นหลังที่เลือก</span>
                </span>
                <span class="text-[10px] bg-cyan-500/30 border border-cyan-400/40 px-2 py-0.5 rounded-md font-black text-cyan-300">
                  กำลังใช้งาน
                </span>
              </div>
            </div>
          ...
          <!-- Buttons -->
          <div class="flex gap-2.5 pt-3 border-t border-slate-100 dark:border-[#212327]">
            <button
              @click="showThemeModal = false"
              type="button"
              class="flex-1 bg-slate-100 dark:bg-[#1a1c20] hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-black py-3 rounded-2xl text-xs transition cursor-pointer"
            >
              ยกเลิก
            </button>
            <button
              ...
              class="flex-1 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white font-black py-3 rounded-2xl text-xs transition shadow-lg shadow-cyan-600/25 flex items-center justify-center gap-1.5 cursor-pointer"
            >
              <div v-if="savingTheme" class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              <template v-else>
                <Icon name="lucide:check" class="w-3.5 h-3.5" />
                <span>บันทึกพื้นหลัง</span>
              </template>
            </button>
          </div>
        </div>
