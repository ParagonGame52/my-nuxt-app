...
async function onSlipFileChange(e: Event) {
        ...
        <div class="p-6 space-y-5">
          ...
          <div v-if="slipInputMode === 'upload'">
            ...
            >
              ...
              <div v-else-if="isScanningSlip" class="flex flex-col items-center gap-1.5 text-emerald-500">
                <div class="w-6 h-6 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />
                <p class="text-[10px] font-bold">กำลังตรวจสอบ QR Code...</p>
              </div>
              <div v-else class="flex flex-col items-center gap-1 text-slate-400">
                <Icon name="lucide:upload-cloud" class="w-7 h-7" />
                <p class="text-[10px] font-bold">คลิกเลือกรูปสลิป (ต้องมี QR Code)</p>
              </div>
            </label>
            <input id="slip-upload" type="file" accept="image/*" class="hidden" :disabled="isScanningSlip" @change="onSlipFileChange" />
          </div>
