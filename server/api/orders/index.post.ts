export default defineEventHandler(async (event) => {
  ...
  `).all(user.id) as { product_id: number; product_name: string; product_price: number; quantity: number }[]
  ...
  const processedItems: { product_id: number; product_name: string; product_price: number; quantity: number }[] = []
  for (const item of cartItems) {
    const p = await db.prepare('SELECT stock, name, price, category FROM products WHERE id = ?').get(item.product_id) as { stock: number; name: string; price: number; category: string } | undefined
    if (!p) {
  ...
  if (payment_method === 'wallet') {
    ...
    const executeCheckout = db.transaction(async () => {
      ...
      for (const item of processedItems) {
        insertItem.run(orderId, item.product_id, item.product_name, item.product_price, item.quantity)
