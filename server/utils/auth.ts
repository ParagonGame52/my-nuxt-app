...
export async function getUserFromEvent(event: H3Event): Promise<SessionUser | null> {
  ...
  const session = await db.prepare(`
    SELECT s.user_id, u.name, u.email, u.balance, u.is_admin, u.shipping_name, u.shipping_phone, u.shipping_address, u.avatar, u.banner_url, u.banner_theme, u.points, u.total_spent, u.tier
    FROM sessions s
  ...
  return {
    ...
    banner_theme: session.banner_theme || '',
